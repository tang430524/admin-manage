...
public class NavController {
    ...
    public String toform(@RequestParam(value = "id", required = false) String id, @RequestParam(value = "parent", required = false) boolean parent, Model model) {
        ...
        if (StringUtils.isNotBlank(id) && !parent) {
            model.addAttribute("menu", restTemplate.getForObject("/admin/nav/{id}"));
            url = "/menu/" + id + "/modify";
        } else {
            url = "/menu/add";
        ...
        return "blogsite/menu/form";
    }

    @RequestMapping(method = RequestMethod.GET)
    public String list(Model model) {
        model.addAttribute("list", restTemplate.getForObject("/admin/nav/list"));
        return "blogsite/menu/list";
    }
